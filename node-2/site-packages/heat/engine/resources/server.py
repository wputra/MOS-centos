...
class Server(stack_user.StackUser):
    ...
    def validate(self):
        ...
        # limits
        if metadata is not None and limits['maxServerMeta'] != -1:
        ...
        if personality is not None:
            if limits['maxPersonality'] != -1:
                if len(personality) > limits['maxPersonality']:
                    msg = _("The personality property may not contain "
                            "greater than %s entries.") % \
                        limits['maxPersonality']
                    raise exception.StackValidationFailed(message=msg)

            for path, contents in personality.items():
                if len(bytes(contents)) > limits['maxPersonalitySize'] and \
                        limits['maxPersonalitySize'] != -1:
